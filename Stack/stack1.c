...
=> Algorithm of void push(int no)
    ...
    step-2 : if stack is full then produce an error
             stack overflow and exit
    step-3 : if stack is not full then increment
             top of stack by 1
...
=> Algorithm of void pop()
    ...
    step-3 : if stack is not empty then
...
-> The index value is subtracted from top + 1
...
=> Algorithm of void peep(int location)
    ...
    step-3 : if stack is empty then produce an error
             stack underflow and exit
    step-4 : if stack is not empty then perform
             operation index = top - location + 1
...
5) top pointer/indicator -> it is use to manage stack

6) condition of stack overflow and stack overflow
